...
lim_P2=80 # Limit phase 2
lim_P3=110 # Limit phase 3
...
HH_obs$N_days=as.integer(HH_obs$Date-HH_obs$Date[1])
...
Srange=c(seq(1,9) %o% 10^(-3:-2)) # Investigated range of parameters
Krange=c(seq(1,9) %o% 10^(-6:-5))
...
P3_opt <- function(x){
  ...
  alpha= K*b_mean/S
  Impulse_P3=as.data.frame(Tab_inf$Time_sec)
  colnames(Impulse_P3)=c('Time_sec')
  Impulse_P3$d=sqrt(4*alpha*Impulse_P3$Time_sec)
  
  Impulse_P3$dZdt=b_mean/(2*S)*(erf((L/2+X)/Impulse_P3$d)+erf((L/2-X)/Impulse_P3$d))*
    (erf((W/2+Y)/Impulse_P3$d)+erf((W/2-Y)/Impulse_P3$d))
  
  ### CONVOLUTION ### 
  
  for (day in (lim_P2+1):len){
    
    c=0
    i2=min(Impulse_duration,day-1)
    
    for (i_offset in (lim_P2+1):i2){ # Offset timestep
      
      if (i_offset<lim_P3){
        
        Inf_offset=Tab_inf$Inf_m_s[day-i_offset+lim_P2]
      ...
      c=c+Inf_offset*Impulse_P3$dZdt[i_offset]
    }
    
    Output_P3[day]= time_step*c
  }
  
  Output_P3=as.data.frame(Output_P3)
  
  Output_t2=as.data.frame(Output_P3+Output_P2+Output_P1)
  h_t2=sqrt(Output_t2+b^2)
  h_t2["N_days"]=Tab_inf$N_days
  colnames(h_t2)=c("h","N_days")
  
  Results_t2=merge(h_t2,HH_obs[c("N_days",paste("WL_",borewell,sep=""))], by=paste("N_days"),all.x = TRUE)
  Results_t2=as.data.frame(c(Tab_inf["Date"],Results_t2[,c(1,3,2)]))
  
  ### ERROR ###      
  
  RMSE_P3 <- sqrt(mean((Results_t2[1:lim_P3,paste("WL_",borewell,sep="")]-Results_t2$h[1:lim_P3])^2,na.rm=TRUE))
  
  c(RMSE_P3,Output_P3,as.data.frame(h_t2$h),as.data.frame(Impulse_P3$dZdt))
...
Tab_RMSE_P3=matrix(0, length(Srange),length(Krange)) # Initialize tables

for (n in 1:length(Srange)){
  
  for (m in 1:length(Krange)){
    
    res=P3_opt(c(Srange[n],Krange[m]))
    
    Tab_RMSE_P3[n,m]=res[[1]]
    
  }
}

P3_K=Krange[which(Tab_RMSE_P3 == min(Tab_RMSE_P3), arr.ind = TRUE)[2]] # Optimal parameters
P3_S=Srange[which(Tab_RMSE_P3 == min(Tab_RMSE_P3), arr.ind = TRUE)[1]]
...
Res_t2=P3_opt(c(P3_S,P3_K))
h_t2=as.data.frame(Res_t2[[3]])
h_t2["N_days"]=Tab_inf$N_days  

Output_P3=as.data.frame(Res_t2[[2]])
h_P3=sqrt(as.data.frame(Output_P3)+b^2)
h_P3["N_days"]=Tab_inf$N_days  

Tab_res_t2=merge(merge(h_P1,merge(h_t,merge(h_P2,merge(h_P3,h_t2,by="N_days"),by="N_days"),by="N_days"),by="N_days"),HH_obs[c("N_days",paste("WL_",borewell,sep=""))], by="N_days",all.x = TRUE)
colnames(Tab_res_t2)=c("N_days","h_P1","h_t","h_P2","h_P3","h_t2",paste("WL_",borewell,sep=""))
Tab_res_t2=as.data.frame(c(Tab_inf["Date"],Tab_res_t2[,c(1,7,2,4,5,3,6)]))
...
# #   Results_t2=merge(Results_t2,HH_obs[c(4,5)],by="N_days")
# #   Results_t2['Obs']=Results_t2[paste("WL_",borewell_list[b_num],sep="")]+HH_obs[paste("WL_",borewell_list[b_num],sep="")][lim_P2,1]
# #   
# #   RMSE_P3=sqrt(mean((Results_t2$Obs[lim_P2:len]-Results_t2$h_P3[lim_P2:len])^2,na.rm=TRUE))
# #   
...
# # 
# # Tab_res_t2=Tab_res_t2[c(2,1,3,4,5,8,6,7)]
# 
# 
# 
# ############
...
# 
# x11()
# yrange <- c(min(range(Tab_res_final[paste("WL_",borewell,sep="")],na.rm = TRUE)[1],range(Tab_res_final["h_P2"])[1]),
#             max(range(Tab_res_final[paste("WL_",borewell,sep="")],na.rm = TRUE)[2],range(Tab_res_final["h_P2"])[2]))
