...
dir_in="C:/Users/Madeleine/Documents/Doctorat/ARTICLE/Data_Groundwater mound/Data"
dir_out="C:/Users/Madeleine/Documents/Doctorat/ARTICLE/Data_Groundwater mound/Graphs"

inf_file_name="Data_inf.txt" # Infiltration 
profile_file_name="Data_profile_coord3.txt" # Profile coordinates
...
# Mound_profile_topo=as.data.frame(t(Mound_profile))
# Mound_profile_topo=as.data.frame(t(Mound_profile_topo+profile_coord$Fisslay_depth))

output_name <- readline(prompt="Enter output file name: ")

write.xlsx(Mound_profile, paste(output_name,".xlsx",sep=""))
...
# pdf(paste("GW_mound_evolution_topo",".pdf",sep=""))
# 
# for (i in 1:244){
#   
#   yrange <- range(Mound_profile_topo)
#   
#   xrange <- range(profile_coord$FID)
#   
#   # set up the plot
#   
#   plot(xrange, yrange, type="n", xlab="Position",
#        ylab="Groundwater mound (m)" )
#   colors <- "#000000"
#   linetype <- 1
#   plotchar <- 3 # Plotting symbol
#   
#   lines(profile_coord$FID, Mound_profile_topo[i,], type="b", lwd=1.5,
#         lty=linetype, col=colors, pch=plotchar)
# }
# 
# dev.off()
